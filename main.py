...
from core.ai_agentic import DocumentAIAgent
from core.ollama_llm import ModelManager, validate_ollama_setup
...
# Initialize AI agent with proper error handling
try:
    ...
    if ai_agent.is_ready():
        ...
        logger.warning("⚠ AI Agent initialized but not fully ready")
    ...
    try:
        ...
        if not ai_agent or not ai_agent.is_ready():
            return JSONResponse(
                status_code=503,
                content={
                    "message": "AI Agent is not ready. Please check system status.",
                    "failed_files": [f["filename"] for f in failed_files] + uploaded_files
                }
            )
...
async def ask_question(request: QuestionRequest):
    ...
    try:
        ...
        result["model_used"] = config.get("ollama.model", "gemma:latest")
...
async def manage_models(request: ModelRequest):
    ...
    try:
        ...
        if action == "pull":
            success = ModelManager.pull_model(model_name)
            if success:
                return JSONResponse(content={
            ...
            success = ModelManager.delete_model(model_name)
            if success:
                return JSONResponse(content={
